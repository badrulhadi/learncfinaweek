...
<cfif form.submitted>
	<cfif isSimpleValue(form.contactname)>
		<cfset form.contactname=canonicalize(form.contactname, true, true)/>
	</cfif>
	<cfif isSimpleValue(form.comment)>
		<cfset form.comment=canonicalize(form.comment, true, true)/>
	</cfif>
	<cfif !isSimpleValue(form.contactname) || !isSimpleValue(form.comment) || !CSRFVerifyToken(form.token)>
						...
						<cfoutput>
							...
							<div class="clr postComment">
								<form action="blogpost.cfm?id=#blogPost.id#" method="post" id="form">
									<div>
										<label>Name
											<span class="font-11">(required)</span></label>
